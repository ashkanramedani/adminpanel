...
export class SalaryPolicyComponent implements OnInit {
  ...
  {
    // this.http.getAll(`${Domain.GetCount}?field=SalaryPolicy`).subscribe((response)=>
    // {
  ...
  GetResponseData(page:number,limit:number,order:string) {
    this.isLoading=true
    this.http.getAll(`${Domain.GetSalaryPolicyData}?page=${page}&limit=${limit}&order=${order}`).subscribe((response) => {
  ...
  RemoveItem(id?: number) {
    this.alertServices.confirm(
      ...
      () => {
        this.http
        .deleteWithQuery(`${Domain.DeleteSalaryPolicyData}/${id}`)
        .subscribe((response) => {
          console.log(response);
          if (response == "Deleted") {
            this.GetResponseData(1, 10, this.order);
  ...
  OpenModal(id: string) {
    ...
    this.http
      .get(Domain.GetSalaryPolicyData, id)
      .subscribe((response) => {
